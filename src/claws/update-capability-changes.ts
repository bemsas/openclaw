...
function capabilityValue(value: unknown, summary: string): ClawUpdateCapabilityValue {
  return {
    summary,
    digest: `sha256:${createHash("sha256").update(stableStringify(value)).digest("hex")}`,
  };
}
...
export function pushAgentCapabilityChanges(params: {
  ...
  for (const field of fields) {
    ...
    const classification = classifyAgentCapability(path, current, desired);
    params.changes.push({
      kind: "agent",
      id: params.agentId,
      path: `agent.${path}`,
      ...
      ...(current === undefined
        ? {}
        : { current: capabilityValue(current, summarizeAgentCapability(current)) }),
      ...(desired === undefined
        ? {}
        : { desired: capabilityValue(desired, summarizeAgentCapability(desired)) }),
    });
  }
}
...
export function pushResolvedAgentCapabilityChanges(params: {
  ...
  const desiredConfig: OpenClawConfig = {
    ...
    agents: {
      ...
      list: [
        ...(params.config.agents?.list ?? []).filter((agent) => agent.id !== params.agentId),
        params.desiredAgent,
      ],
    },
  };
  pushAgentCapabilityChanges({
    ...
    currentAgent: params.config.agents?.list?.find((agent) => agent.id === params.agentId),
    desiredAgent: params.desiredAgent,
    currentSandbox: resolveSandboxConfigForAgent(params.config, params.agentId),
    desiredSandbox: resolveSandboxConfigForAgent(desiredConfig, params.agentId),
    currentHeartbeat: resolveHeartbeat(params.config, params.agentId),
...
export function mcpCapabilityChange(params: {
  ...
  return {
    ...
    ...(params.current === undefined
      ? {}
      : {
          current: capabilityValue(params.current, summarizeMcpCapability(params.current)),
        }),
    ...(params.desired === undefined
      ? {}
      : {
          desired: capabilityValue(params.desired, summarizeMcpCapability(params.desired)),
        }),
  };
}
...
export function cronCapabilityChange(params: {
  ...
  return {
    ...
    ...(params.current === undefined
      ? {}
      : {
          current: capabilityValue(params.current, summarizeCronCapability(params.current)),
        }),
    ...(params.desired === undefined
      ? {}
      : {
          desired: capabilityValue(params.desired, summarizeCronCapability(params.desired)),
        }),
  };
}
